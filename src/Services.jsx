...
const suffix = match ? value.slice(match[0].length) : '';
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting) {
          ...
          // Disconnect after starting animation
          observer.disconnect();
        }
      },
      { threshold: 0.1 }
  ...
  return (
    <span ref={ref}>
      {isPercentage ? `${count}${suffix}` : count}{!isPercentage && suffix}
    </span>
  );
};
...
const Services = () => {
  ...
  const downloadQuotation = () => {
    const pdfUrl = '/Sagus Solutions Quotation.pdf';
    const link = document.createElement('a');
    link.href = pdfUrl;
    link.download = 'DigitalAptech-Quotation.pdf';
    document.body.appendChild(link);
                  ...
                  <div className="flex items-start justify-between mb-4 md:mb-6">
                    ...
                    <div className="text-right">
                      <div className="text-xl md:text-2xl font-bold text-slate-900">
                        <AnimatedCounter value={service.stats} duration={1500} />
                      </div>
                      <div className="text-xs text-slate-500 font-medium">{service.metric}</div>
                    </div>
                  </div>
